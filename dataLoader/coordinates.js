...
async function readSedesFile() {
  try {
    const data = await fs.readFile("../dataProc/sedes.json", "utf-8");
...
async function getCoordinates(sede) {
  ...
  if (!direccion || !localidad || !provincia) {
    console.log(
      `Skipping sede ${sede.id} due to incomplete address information`
    );
    return null;
  }
  ...
  try {
    ...
    console.error(
      `Error getting coordinates for sede ${sede.id}:`,
      error.message
    );
    console.log(error);
    process.exit(1);
...
async function processSedesAndSaveCoordinates() {
  try {
    ...
    await fs.writeFile(
      "coordinates.json",
      JSON.stringify(validCoordinates, null, 2)
    );
